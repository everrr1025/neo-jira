...
import { ArrowLeft, Crown, Loader2, Plus, Search, Trash2, UserPlus, X } from "lucide-react";
...
const TEXT = {
  en: {
    ...
    subtitle: "Click a member to make them department head.",
  ...
  const handleAddSelectedMembers = () => {
    ...
    startTransition(async () => {
      for (const userId of selectedMemberIds) {
        const res = await addMemberToDepartment(department.id, userId);
        if (!res.success) {
          setErrorMsg(res.error || t.assignFailed);
          return;
        }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <Link
            ...
            className="mb-3 inline-flex items-center gap-1 text-sm font-medium text-slate-500 transition-colors hover:text-blue-600"
          >
            <ArrowLeft size={16} />
            {t.back}
          </Link>
