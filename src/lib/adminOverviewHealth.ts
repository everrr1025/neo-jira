...
export function buildUsageHealth(users: UsageHealthUser[], days: 30 | 90, now: Date): UsageHealthSummary {
  ...
  const attentionDepartments = [...departments.values()]
    .map((department) => ({
      ...department,
      activeRate: Math.round((department.activeUsers / department.eligibleUsers) * 100),
    }))
    .filter((department) => department.activeUsers / department.eligibleUsers < ATTENTION_ACTIVE_RATE)
